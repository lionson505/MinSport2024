...
export default function FootballScoreboard({ match, teamAPlayers = [], teamBPlayers = [], onUpdate }) {
  ...
  const addEvent = async (type, team, matchId, player = null) => {
    try {
      ...
      const currentMinute = parseInt(matchData.currentTime.split(':')[0], 10);
      ...
      if (type === 'GOAL') {
        ...
        if (!playerId || isNaN(currentMinute) || !cardType) {
          throw new Error('Missing required fields for card event: nationalTeamPlayerStaffId, minute, and type.');
        }
        ...
        toast.success(`${cardType === 'YELLOW' ? 'Yellow' : 'Red'} card event posted successfully!`, {
          description: `${cardType === 'YELLOW' ? 'Yellow' : 'Red'} card for player ID: ${playerId} at minute: ${currentMinute}`
        });
