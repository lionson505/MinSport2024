...
const Training = () => {
  ...
  // Filter trainings based on search query
  useEffect(() => {
    console.log('Filtering trainings with query:', searchQuery);
    const filtered = trainings.filter((training) =>
        training.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  // Handle adding or editing training
  const handleAddOrEditTraining = async (data) => {
    console.log('Handling add/edit training:', data);
    setIsSubmitting(true);
    try {
      let response;
      if (trainingToEdit) {
        console.log('Editing training:', trainingToEdit.id);
        response = await axiosInstance.put(`/trainings/${trainingToEdit.id}`, data);
        const updatedTrainings = trainings.map((t) =>
            t.id === trainingToEdit.id ? response.data : t
        );
      ...
      console.error('Error adding/editing training:', error);
      setMessage({
        type: 'error',
        text: 'Failed to add or update training'
      });
  ...
  // Handle deleting a training
  const handleDeleteConfirm = async () => {
    console.log('Deleting training:', trainingToDelete.id);
    setIsSubmitting(true);
    try {
      await axiosInstance.delete(`/trainings/${trainingToDelete.id}`);
      const updatedTrainings = trainings.filter(t => t.id !== trainingToDelete.id);
  ...
  const canCreateTraining = checkPermission(MODULE_IDS.TRAININGS, 'create');
  console.log('Can create training:', canCreateTraining);
