...
// Add custom class for consistent input sizing
const inputClassName = "h-12 w-full"; // Increased height to 48px (h-12)
...
const EditEventModal = ({ isOpen, onClose, event, onEventUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/sports-tourism-events/${event.id}`, formDataToSend, {
        headers: {
  ...
  return (
    <>
      ...
      <Modal
        isOpen={isOpen}
        onClose={onClose}
        title="Edit Sports Tourism Event"
        size="2xl"
      >
